...
| Skill | What it does |
|---|---|
| `uiux/ux-principles` | Framework- and style-agnostic UX discipline - decides whether an interface *works*: visual hierarchy, chunking, Gestalt grouping, scale/contrast/signal-to-noise, a decision toolkit of UX laws (Hick, Fitts, etc), progressive disclosure, forgiveness, full interaction-state cycles, and accessibility as a floor. Applies to any surface - marketing, product, dashboard, form. |
| `uiux/ui-taste` | Anti-slop *visual* UI design for product and marketing surfaces. Reads the brief instead of defaulting to a templated look, then builds around a normalized **style contract**. Goes beyond avoiding tells to visual-aesthetic reasoning: a **governing concept** that gives the surface a soul (interpreted from audience/brand, not looked up from clichés), color harmony, and hierarchy as aesthetic rhythm. The aesthetic is pluggable: pick a built-in style (`minimalist`, `brutalist`, etc), describe your own, hand over a reference image, or let it infer one. Enforces style-independent invariants that no style may override, plus an AI-tells catalog and a pre-flight check. |
| `uiux/ui-style-guide` | Systematizes a resolved visual language into a shareable **style guide / design system**: a token taxonomy (color roles, type scale with size/line-height, spacing, etc), a component inventory showing every component in every state , and grid/spacing/iconography specimens. Also judges *whether a project needs a system at all* vs. a lightweight guide. |

> `uiux/ux-principles` (does it *work*) and `uiux/ui-taste` (how it *looks and feels*) are designed to pair, but each works standalone and requires neither the other nor any framework skill. `uiux/ui-taste` and `uiux/ui-style-guide` pair especially closely: `ui-taste` decides the visual language, `ui-style-guide` documents and componentizes it. New visual styles for `ui-taste` are pluggable: drop a style pack into `uiux/ui-taste/styles/` following `_template.md`.
